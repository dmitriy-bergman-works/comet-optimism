...
export default migration('1713517203_configurate_and_ens', {
  ...
  enact: async (deploymentManager: DeploymentManager, govDeploymentManager: DeploymentManager) => {
    ...
    const depositToCalldata = utils.defaultAbiCoder.encode(
      ['address'],
      [comet.address]
    );
    ...
    const mainnetActions = [
      // 1. Set Comet configuration and deployAndUpgradeTo new Comet on Arbitrum.
      {
        ...
        value: createRetryableTicketGasParams.deposit.add(WETHAmountToBridge)
      },
      ...
      },
      {
        contract: WETH,
        signature: 'approve(address,uint256)',
        args: [arbitrumL1GatewayRouter.address, WETHAmountToBridge]
      },
      ...
      //   args: [
      //     WETH.address,                             // address _token,
      //     refundAddress,                            // address _refundTo
      //     comet.address,                            // address _to,
      //     WETHAmountToBridge,                       // uint256 _amount,
      //     wethGasParams.gasLimit,                   // uint256 _maxGas,
      //     wethGasParams.maxFeePerGas,               // uint256 _gasPriceBid,
      //     utils.defaultAbiCoder.encode(
      //       ['uint256', 'bytes'],
      //       [wethGasParams.maxSubmissionCost, '0x']
      //     )                                         // bytes calldata _data
      //   ],
      //   value: wethGasParams.deposit.add(WETHAmountToBridge)
      // },
  ...
  async verify(deploymentManager: DeploymentManager, govDeploymentManager: DeploymentManager) {
    ...
    // 1.
    console.log(wstETH.address, WETH.address, rETH.address);
